...
const agents = [
  ...
  { role: "Reviewer", desc: "Checks the draft against the source before it ships." },
  { role: "Writer", desc: "Produces the report a person actually reads." },
];
 ...
 * wipes in — the headline's claim replaced by the thing that backs it up.
 */
function useHeroAnimation(reducedMotion: boolean) {
  ...
  useGSAP(
    () => {
      ...
      if (reducedMotion) {
        // No pin, no scrub, no parallax — everything at rest, one fade in.
      ...
      scrub
        .to(
          headline,
          { scale: 0.42, y: liftToTop, opacity: 0.9, ease: "none" },
          0
        )
    ...
    >
      <div className={styles.stage}>
        <div className={styles.topBar} data-topbar>
          ...
          <HudTag>19.0330° N / 73.0297° E</HudTag>
        </div>
        ...
        <div className={styles.pipeline} data-pipeline>
          <div className={styles.pipelineNodes}>
            {agents.map((agent) => (
              <HudFrame key={agent.role} className={styles.node}>
          ...
          <div className={styles.pipelineFoot}>
            <p className={styles.lead}>
              I build agentic AI pipelines. The three above run in production at
              Angel One, writing personalised portfolio reports —{" "}
              <span className={styles.leadStrong}>
                click-through up 120%, opens up 60%.
              </span>
            </p>
